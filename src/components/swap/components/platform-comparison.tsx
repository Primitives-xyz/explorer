...
export function PlatformComparison({
  ...
  const platforms = useMemo(() => {
    ...
    const baseOutputAmount = jupiterSwapResponse && platformExpectedOutAmount ? 
      Number.parseFloat(platformExpectedOutAmount) : 0

    // Add SSE platform first
    allPlatforms.push({
      name: 'SSE',
      outputAmount: baseOutputAmount ? formatSmartNumber(baseOutputAmount, {
    ...
    allPlatforms.push({
      name: 'Jupiter',
      outputAmount: baseOutputAmount ? formatSmartNumber(baseOutputAmount * 0.99, {
        minimumFractionDigits: 2,
        maximumFractionDigits: outputTokenDecimals,
      }) : '',
      feePercentage: 1
    ...
    allPlatforms.push({
      name: 'BullX',
      outputAmount: baseOutputAmount ? formatSmartNumber(baseOutputAmount * 0.97786, {
        minimumFractionDigits: 2,
        maximumFractionDigits: outputTokenDecimals,
      }) : '',
      feePercentage: 2.214
    ...
    allPlatforms.push({
      name: 'Photon',
      outputAmount: baseOutputAmount ? formatSmartNumber(baseOutputAmount * 0.956, {
        minimumFractionDigits: 2,
        maximumFractionDigits: outputTokenDecimals,
      }) : '',
      feePercentage: 4.4
    ...
    allPlatforms.push({
      name: 'Axiom',
      outputAmount: baseOutputAmount ? formatSmartNumber(baseOutputAmount * 0.94, {
        minimumFractionDigits: 2,
        maximumFractionDigits: outputTokenDecimals,
      }) : '',
      feePercentage: 6
    ...
    return baseOutputAmount 
      ? allPlatforms.sort((a, b) => Number.parseFloat(b.outputAmount) - Number.parseFloat(a.outputAmount))
      ...
      <div className="space-y-2">
        {platforms.map((platform, index) => {
          ...
          return (
            <div
              key={platform.name}
              className={cn("flex items-center justify-between rounded-lg py-1.5 px-2", {
                "border border-primary bg-primary/5": platform.name === "SSE"
              })}
            >
              <div className="flex items-center gap-2">
                ...
                <div className="flex flex-col">
                  ...
                  {platform.name === "SSE" && (
                    <span className="text-xs text-primary font-medium">Best price</span>
                  )}
                </div>
              </div>
