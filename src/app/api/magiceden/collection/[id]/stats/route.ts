...
export async function GET(req: NextRequest, context: RouteContext) {
    try {
        ...
        const holdersStatsApiRes = await fetch(`https://api-mainnet.magiceden.dev/v2/collections/${id}/holder_stats`, options)
        ...
        if (collectionStatsApiResData || holdersStatsApiResData) {
            return NextResponse.json({
                floorPrice: Math.round(collectionStatsApiResData.results.floorPrice / LAMPORTS_PER_SOL * 1000) / 1000,
                listedCount: collectionStatsApiResData.results.listedCount,
                volume24hr: Math.round(collectionStatsApiResData.results.volume24hr / LAMPORTS_PER_SOL * 100) / 100,
                txns24hr: collectionStatsApiResData.results.txns24hr,
                volumeAll: Math.round(collectionStatsApiResData.results.volumeAll / LAMPORTS_PER_SOL * 100) / 100,
                supply: holdersStatsApiResData.totalSupply,
                holders: holdersStatsApiResData.uniqueHolders,
            });
        } else {
            return NextResponse.json(
                { error: 'Invalid collection symbol' },
            );
        }
    } catch (error: any) {
