...
export function processSwapTransaction(transaction: Transaction): ProcessedTransaction {
    ...
    transaction.nativeTransfers.forEach(transfer => {
      ...
      if (transfer.fromUserAccount === feePayer) {
        if (!outgoingTokens[SOL_MINT]) {
          outgoingTokens[SOL_MINT] = { mint: SOL_MINT, amount: amount, symbol: 'SOL' }
        } else {
      ...
      if (transfer.toUserAccount === feePayer) {
        if (!incomingTokens[SOL_MINT]) {
          incomingTokens[SOL_MINT] = { mint: SOL_MINT, amount: amount, symbol: 'SOL' }
        } else {
  ...
  if (transaction.tokenTransfers && transaction.tokenTransfers.length > 0) {
    ...
    const lastIncomingTransfer = [...transaction.tokenTransfers].reverse().find(
  ...
  // Format date/time
  const formattedDate = new Date(transaction.timestamp * 1000).toLocaleDateString()
  const formattedTime = new Date(transaction.timestamp * 1000).toLocaleTimeString()
...
export function getDisplayOutgoingToken(outgoingTokens: { [mint: string]: ProcessedTokenTransfer }, primaryToken: ProcessedTokenTransfer | null = null): ProcessedTokenTransfer | null {
...
export function getDisplayIncomingToken(incomingTokens: { [mint: string]: ProcessedTokenTransfer }, primaryToken: ProcessedTokenTransfer | null = null): ProcessedTokenTransfer | null {
  // If there's a primary token, use that
  if (primaryToken) return primaryToken
...
export function getSwapPairsFromTokenTransfers(transaction: Transaction): Array<{
  signer: string,
  amountA: number,
  symbolA: string,
  mintA: string,
  ...
  const feePayer = transaction.feePayer;
  const transfers = transaction.tokenTransfers?.map(tt => ({
  ...
  for (let i = 0; i < transfers.length; i++) {
    ...
    for (let j = 0; j < transfers.length; j++) {
      ...
      if (inc.mint !== out.mint && (inc.to === feePayer || inc.from === out.to)) {
        swaps.push({
          signer: feePayer,
          amountA: out.amount,
...
export function getTokenFeeTransfers(transaction: Transaction) {
  if (!transaction.tokenTransfers || transaction.tokenTransfers.length === 0) return [];
