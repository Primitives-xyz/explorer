...
import {
  HIGHLIGHT_COLORS,
  useAddressHighlight,
} from '@/components/common/use-address-highlight'
...
export function SolanaAddressDisplay({
  ...
  const displayElement = (
    ...
    >
      {displayText ||
        (displayAbbreviatedAddress ? abbreviatedAddress : address)}
    </span>
  )

  // Show full address on hover if tooltip is enabled
  const wrappedDisplay =
    fullAddressOnHover && showTooltip ? (
      <TooltipProvider>
        <Tooltip delayDuration={400}>
          <TooltipTrigger asChild>{displayElement}</TooltipTrigger>
          <TooltipContent>
            <p className="text-sm break-all max-w-80">
              {isValid ? address : 'Invalid Solana address'}
            </p>
          </TooltipContent>
        </Tooltip>
      </TooltipProvider>
  ...
  if (!showCopyButton) {
    return (
      <span className="relative inline-block" ref={containerSpanRef}>
        {wrappedDisplay}
