...
export function NFTContent({ id }: NFTContentProps) {
  ...
  const displayAttributes = attributes.filter(
    (attr: any) => attr.trait_type !== 'Attributes Count'
  )
  const collection = useMemo(
    () =>
      nft?.grouping?.find((g: any) => g.group_key === 'collection')
        ?.group_value,
    [nft?.grouping]
  )
  ...
  const creators = useMemo(
    () =>
      (nft?.creators || []).sort(
        (a: any, b: any) => (b.share ?? 0) - (a.share ?? 0)
      ),
    [nft?.creators]
  )
          ...
          <div className="mt-4 w-full space-y-3">
            ...
            <div className="pt-2">
              ...
              <div className="flex gap-2 flex-wrap">
                <Button
                  variant={ButtonVariant.OUTLINE}
                  href={`https://explorer.solana.com/address/${id}`}
                  newTab
                ...
                <Button
                  variant={ButtonVariant.OUTLINE}
                  href={`https://magiceden.io/item-details/${id}`}
                  newTab
                  className="text-xs py-1 h-auto"
                >
                  Magic Eden
